...
descriptor::descriptor(size_t off_from_base, uint32_t source,
                       uint32_t destination, uint32_t next_desc_ptr,
                       uint32_t bytes_to_transfer, uint32_t write_offset)
{
...
void
descriptor::descriptor_reset(uio_device uio_dev)
{
  uio_dev.uio_write(d_base + DESC_CS_REG, 0x80000000);
}

void
descriptor::write_to_mem(uio_device uio_dev)
{
...
uint8_t
descriptor::read_status(uio_device uio_dev)
{
  return uio_dev.uio_read(d_base + DESC_CS_REG) >> 24;
}

uint16_t
descriptor::read_bytes_txed(uio_device uio_dev)
{
  return uio_dev.uio_read(d_base + DESC_CS_REG) && 0xff;
}

uint32_t
descriptor::get_write_offset()
{
  return d_write_offset;
}

uint32_t
descriptor::get_mem_offset()
{
